...
const BASE_PATH = "./circuits/Customcircuit1/";
...
async function generateProof() {
  ...
  const proof = await snarkjs.groth16.prove(
    BASE_PATH + "out/Customcircuit1.zkey",
    BASE_PATH + "out/circuit.wtns"
  )
...
async function main() {
  // deploy contract
  const Verifier = await ethers.getContractFactory("./contracts/Customcircuit1Verifier.sol:Verifier");
